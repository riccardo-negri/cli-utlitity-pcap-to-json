...
use crate::bmp_parser::parse_bmp_data;
use crate::flow_parser::parse_flow_data;
use bytes::BytesMut;
use chrono::Utc;
use clap::Parser;
use clap::ValueEnum;
...
async fn load_pcap(
    ...
    for (src_ip, src_port, dst_ip, dst_port, protocol, packet_data) in PcapIter::new(pcap_reader) {
        ...
        match config.protocol {
            ProtocolToParse::FlowInfo => {
                parse_flow_data(config.flow_port1, config.flow_port2, flow_key, protocol, &packet_data, &mut exporter_peers, &tx).await
            }
            ProtocolToParse::BMP => {
                parse_bmp_data(config.bmp_port, flow_key, protocol, &packet_data, &mut exporter_peers, &tx).await
            }
        }
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let handle = tokio::spawn(serializer::serialize_data_to_jsonl(rx, config.output_path.clone()));
